...
public class Board {
	...
	public void populateBoard()
	{
		...
		for (int j = 0; j <= 7; j++)
		{
			mainBoard[1][j] = new Pawn(WHITE);
			mainBoard[6][j] = new Pawn(BLACK);
		}
		//populate spaces
		for (int i = 2; i < 6; i++)
		{
			for (int j = 0; j < 8; j++)
			{
				mainBoard[i][j] = new Piece(BLANK);
			}
		}
		mainBoard[1][7] = new Piece(BLANK);
		mainBoard[6][7] = new Piece(BLANK);
	}
		...
		if (x1 > -1 && x1 < 8 && y1 > -1 && y1 < 8)
		{
			if (this.isValidMove(x1, y1, x2, y2))
			{
				...
				else
				{
					...
					mainBoard[x1][y1] = new Piece(BLANK);
				}
			}
			else
			{
	...
	public boolean checkPath(int x1, int y1, int x2, int y2)
	{
		...
		if (x1 == x2)
		{
			...
			else
			{
				...
				{
					if (mainBoard[x1][i].getTeam() != BLANK)
					{
						return false;
					}
				}
		...
		else
		{
			...
			if (difx > 0)
			{
				...
				if (dify > 0)
				{
					...
					{
						if (mainBoard[x1+i][y1+i].getTeam() != 2)
						{
							return false;
						}
					}
